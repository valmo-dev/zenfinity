...
<template>
  <div class="relative">
    ...
    <Transition name="dropdown">
      <div
        v-if="showDropdown"
        class="absolute z-50 w-full mt-1 py-1 bg-base-100 border border-base-content/[0.06] rounded max-h-48 overflow-y-auto"
      >
        <button
          v-for="(cat, index) in suggestions"
          :key="cat"
          type="button"
          class="w-full text-left px-4 py-2 text-sm transition-colors border-b border-base-content/[0.06] last:border-b-0"
